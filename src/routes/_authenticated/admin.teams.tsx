...
export const Route = createFileRoute("/_authenticated/admin/teams")({
  head: () => ({
    meta: [{ title: "Equipos — AliPicks Admin" }, { name: "robots", content: "noindex" }],
  }),
...
function TeamsManager() {
  ...
  const { data: teams = [] } = useQuery<TeamRow[]>({
    ...
    queryFn: async () => {
      let q = supabase.from("teams").select("*, league_teams(league_id)").order("name");
      if (sport !== "all") q = q.eq("sport", sport);
      const { data, error } = await q;
      if (error) throw error;
      return (data ?? []).map((team) => ({
        ...team,
        league_ids: (team.league_teams ?? []).map((item) => item.league_id),
      }));
    },
  });
  ...
  const draftLeagues = useLeagues(draft.sport).data ?? [];
  function openNew() {
    setEditing(null);
    setDraft(emptyDraft());
    setOpen(true);
  ...
  async function save() {
    ...
    if (editing) {
      ...
      if (error) {
        ...
        toast.error(
          error.code === "23505"
            ? "Ya existe un equipo con ese slug."
    ...
    if (!teamId) {
      setBusy(false);
      toast.error("No se pudo resolver el equipo guardado.");
      return;
    }
    ...
    if (deleteError) {
      setBusy(false);
      toast.error("El equipo se guardó, pero no se pudieron actualizar sus ligas.");
      return;
    }
    const { error: linkError } = await supabase
      .from("league_teams")
      .insert(draft.league_ids.map((league_id) => ({ league_id, team_id: teamId! })));
    setBusy(false);
    if (linkError) {
      toast.error("El equipo se guardó, pero falló la asignación de liga.");
      return;
    }
  ...
  async function remove(team: TeamRow) {
    if (
      !window.confirm(`¿Eliminar ${team.name}? Solo será posible si no tiene picks relacionados.`)
    )
      return;
    const { error } = await supabase.from("teams").delete().eq("id", team.id);
    if (error) {
      toast.error("No se puede eliminar el equipo mientras tenga picks relacionados.");
      return;
    }
  ...
  function toggleLeague(id: string) {
    setDraft((d) => ({
      ...d,
      league_ids: d.league_ids.includes(id)
        ? d.league_ids.filter((value) => value !== id)
        : [...d.league_ids, id],
    }));
  }
  return (
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Administra nombres, escudos y pertenencia a ligas.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {filtered.map((team) => (
          ...
          >
            ...
            <Button size="icon" variant="ghost" onClick={() => remove(team)}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Nombre corto">
                <Input
                  ...
                  onChange={(e) => setDraft((d) => ({ ...d, short_name: e.target.value }))}
                  placeholder="MCI"
                />
              </Field>
              <Field label="Slug">
              ...
              <Field label="Deporte">
                <Select
                  value={draft.sport}
                  onValueChange={(v) =>
                    setDraft((d) => ({ ...d, sport: v as Sport, league_ids: [] }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                {draftLeagues.map((league) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={draft.league_ids.includes(league.id)}
